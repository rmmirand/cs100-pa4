 ...
 *
 * The file contains the build file for the graph and the methods for weighted
 * and unweighted pathfinder, uncollaborated and collaborated linkpredictor and
 * the movie travelor method.
 *
...
bool ActorGraph::loadFromFile(const char* in_filename,
    ...
    while (infile) {
        ...
        unordered_map<string, Actor*>::const_iterator actNode =
            actMap.find(actor);
        unordered_map<string, Movie*>::const_iterator movNode =
            movMap.find(movie_title + to_string(year));
        ...
        if (movNode == movMap.end()) {
            ...
            if (theMovie->year != year) {
                ...
                theMovie = newMovie;
                movMap.insert(
                    make_pair(movie_title + to_string(year), newMovie));
            }
        }
...
vector<Actor*> ActorGraph::linkCollab(Actor* actor) {
    ...
    for (unsigned int i = 0; i < collabs.size(); i++) {
        for (unsigned int j = 0; j < collabs[i]->movies.size(); j++) {
            for (unsigned int k = 0; k < collabs[i]->movies[j]->actList.size();
                 k++) {
                if (collabs[i]->movies[j]->actList[k]->actName !=
                    actor->actName) {
                    for (unsigned int l = 0; l < collabs.size(); l++) {
                        if (collabs[l]->actName != collabs[i]->actName &&
                            collabs[i]->movies[j]->actList[k]->actName ==
                                collabs[l]->actName) {
                            if (!collabs[i]->movies[j]->actList[k]->visited) {
                                collabs[i]->movies[j]->actList[k]->visited =
                                    true;
                                collabs[i]->triangles =
                                    collabs[i]->triangles + 1;
                            }
                        }
                    }
                }
...
vector<Actor*> ActorGraph::linkUncollab(Actor* actor) {
    ...
    for (unsigned int i = 0; i < undesirables.size(); i++) {
        for (unsigned int j = 0; j < undesirables[i]->movies.size(); j++) {
            for (unsigned int k = 0;
                 k < undesirables[i]->movies[j]->actList.size(); k++) {
                if (undesirables[i]->movies[j]->actList[k]->actName ==
                    actor->actName) {
                    undesirables[i]->triangles = undesirables[i]->triangles + 1;
                } else {
                    if (undesirables[i]->movies[j]->actList[k]->actName !=
                        undesirables[i]->actName) {
                        for (unsigned int m = 0; m < undesirables.size(); m++) {
                            if (undesirables[m]->actName == undesirables[i]
                                                                ->movies[j]
                                                                ->actList[k]
                                                                ->actName) {
                                here = true;
                        ...
                        if (!here) {
                            here = false;
                            for (unsigned int n = 0; n < uncollabs.size();
                                 n++) {
                                if (uncollabs[n]->actName == undesirables[i]
                                                                 ->movies[j]
                                                                 ->actList[k]
                                                                 ->actName) {
                                    here = true;
                            ...
                            if (!here) {
                                uncollabs.push_back(
                                    undesirables[i]->movies[j]->actList[k]);
                            }
                        }
                        here = false;
                    }
    ...
    for (unsigned int i = 0; i < uncollabs.size(); i++) {
        for (unsigned int j = 0; j < uncollabs[i]->movies.size(); j++) {
            for (unsigned int k = 0;
                 k < uncollabs[i]->movies[j]->actList.size(); k++) {
                for (unsigned int m = 0; m < undesirables.size(); m++) {
                    if (uncollabs[i]->movies[j]->actList[k]->actName ==
                        undesirables[m]->actName) {
                        if (!undesirables[m]->visited) {
                            uncollabs[i]->triangles =
                                uncollabs[i]->triangles +
                                undesirables[m]->triangles;
                            undesirables[m]->visited = true;
                        } else {
                            uncollabs[i]->triangles =
                                uncollabs[i]->triangles + 1;
                        }
                    }
                }
            }
        ...
        for (unsigned int j = 0; j < allMovies[i]->actList.size(); j++) {
            if (!allMovies[i]->actList[j]->visited) {
                if (j + 1 < allMovies[i]->actList.size()) {
                    ...
                    out << allMovies[i]->actList[j]->actName << "<--["
                        << allMovies[i]->movTit << "#@" << allMovies[i]->year
                        << "]-->" << allMovies[i]->actList[j + 1]->actName
                        << endl;
                } else {
                    if (j - 1 >= 0 && !allMovies[i]->actList[j - 1]->visited) {
                        ...
                        out << allMovies[i]->actList[j]->actName << "<--["
                            << allMovies[i]->movTit << "#@"
                            << allMovies[i]->year << "]-->"
                            << allMovies[i]->actList[j - 1]->actName << endl;
                    }
                }
            }
        }
