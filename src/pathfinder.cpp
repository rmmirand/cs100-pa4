 ...
 *
 *  Pathfinder finds the shorted distance between the two given actors, if the
 * search is unweighted it searches for the path with the least amount of
 * vertices, if weighted, it outputs the path with the shortest weightest total
 * distance.
 */
#include <string.h>
#include <climits>
...
#define ARGS_VAL 5
using namespace std;
// function pathfinder not using weights
void unweightedPath(string inFile, string outFile, ActorGraph* graph) {
    ...
    while (in.peek() != ifstream::traits_type::eof()) {
        ...
        cout << "Computing path for (" << firstActor << ") -> (" << secondActor
             << ")" << endl;

        unordered_map<string, Actor*>::const_iterator actStart =
            ((graph->getactMap()).find(firstActor));
        unordered_map<string, Actor*>::const_iterator actEnd =
            ((graph->getactMap()).find(secondActor));
        total = "";
        if (actStart == (graph->getactMap()).end()) {
            ...
            Actor* secondAct =
                (*((graph->getactMap()).find(secondActor))).second;
            ...
            while (firstAct->actName != firstActor && firstAct->prev) {
                total = "--[" + prevMov->movTit + "#@" +
                        to_string(prevMov->year) + "]-->(" + firstAct->actName +
                        ")" + total;
        ...
        unordered_map<string, Actor*>::iterator resetA =
            ((graph->getactMap()).begin());
        unordered_map<string, Movie*>::iterator resetM =
            ((graph->getmovMap()).begin());
...
// Function for pathfinder using weights
...
int main(int argc, char* arg[]) {
    ...
    // checks if user indicated weighted or unweighted
    if (strcmp(arg[2], "u") == 0) {
